...
而$T(t)=e^{-\int_0^t\sigma(u)du}$需要从起点开始计算，但可以拆分为两段：
...
进一步令$\delta_n=t_{n+1}-t_n$，则$T(0\rightarrow t_n)$也可以离散化：
...
$$I(s)=I_0T(s)+\sum_{n=1}^N C_n T_n \left(1-e^{-\sigma_n\delta_k}\right)$$
