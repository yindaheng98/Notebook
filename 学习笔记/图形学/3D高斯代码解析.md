...
```cpp
	...
	dim3 tile_grid((width + BLOCK_X - 1) / BLOCK_X, (height + BLOCK_Y - 1) / BLOCK_Y, 1); //网格中block块的数量，确保覆盖全图
...
```cpp
	...
	CHECK_CUDA(cub::DeviceScan::InclusiveSum(geomState.scanning_space, geomState.scan_size, geomState.tiles_touched, geomState.point_offsets, P), debug) //计算tiles_touched数组的前缀和，存到point_offsets中
	...
	BinningState binningState = BinningState::fromChunk(binning_chunkptr, num_rendered); //将分配的显存块初始化为BinningState
```
