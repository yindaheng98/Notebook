...
当前像素Loss值$L$对按深度排序的第$i$个高斯点的颜色$C_i$的梯度表示为：
...
$$C=T_{0}\cdot C_{bg}+\sum_{i=1}^{N}T_{i}\cdot\alpha_{i}\cdot C_{i}$$

其中$T_{0}$是alpha-blending运行到最后剩余的透明度，用于把背景颜色$C_{bg}$blending进来；$\alpha_{i}$和$T_{i}$分别对应第$i$个高斯点在当前像素的透明度和按顺序blending到第$i$个高斯点时剩余的透明度。
...
颜色$C$对第$i$个高斯点的透明度 $\alpha_i$ 求导：

$$
\frac{\partial C}{\partial \alpha_i} = \frac{\partial T_{0}}{\partial \alpha_i}\cdot C_{bg}+\sum_{j=1}^{N}\frac{\partial (T_{j}\cdot\alpha_{j})}{\partial \alpha_i}\cdot C_{j}
$$
...
于是，当$j<i$时，$T_j$、$\alpha_j$、$C_j$均与$\alpha_i$无关，梯度为0；当$j=i$时有：
...
令$C^{\text{accum}}_i=\sum_{j=i+1}^{N}\alpha_j\cdot C_j\cdot \prod_{k=i+1}^{j-1} (1 - \alpha_k)$，则有
...
$$
\frac{\partial L}{\partial \alpha_i} = \frac{\partial L}{\partial C} \cdot T_i \cdot (C_i - C^{\text{accum}}_i)
$$
...
可以看出它其实就是在高斯点$i$之后的alpha-blending积累的颜色。所以在代码中对应了一段对积累的颜色逐步拆卸的过程`accum_rec[ch] = last_alpha * last_color[ch] + (1.f - last_alpha) * accum_rec[ch]`
