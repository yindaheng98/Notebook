...
因为对称矩阵是四个值，偏导数本应有四项，$B$占了两个位置，所以偏导数中$-G_i\Delta x \Delta y$写在矩阵里是两个偏导数$-\frac{1}{2}G_i\Delta x \Delta y$。
因为后面[`BACKWARD::preprocess`](./3D高斯反向传播代码解析2.md)里要用矩阵形式计算，所以作者这里保存的是写在矩阵里的形式。
