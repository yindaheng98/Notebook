...
$j=i+1$时$\prod_{k=i+1}^{j-1} (1 - \alpha_k)=\prod_{k=i+1}^{i+1} (1 - \alpha_k)=1-\alpha_{i+1}$
...
$$
\frac{\partial L}{\partial G_i} = \frac{\partial L}{\partial \alpha_i} \cdot \text{opacity}_i
$$
...
高斯函数（高斯点$i$在当前像素上的值$G_i$）定义为（类似二维正态分布的概率密度函数）：

$$
G_i = \exp\left( -\frac{1}{2} \left( A \cdot \Delta x^2 + 2B \cdot \Delta x \Delta y + C \cdot \Delta y^2 \right) \right)
$$

其中，$(\Delta x, \Delta y)^T=(x_i, y_i)^T-(x_{\text{pixel}}, y_{\text{pixel}})^T$为高斯球在成像平面上投影的高斯椭圆中心$(x_i, y_i)^T$到当前像素位置$(x_{\text{pixel}}, y_{\text{pixel}})^T$的向量；$A, B, C$ 是锥体参数（对称的二次型矩阵参数）。

对 $G_i$ 关于 $\Delta x$ 和 $\Delta y$ 求导：

$$
\begin{aligned}
\frac{\partial G_i}{\partial \Delta x}&=-G_i \left( A \cdot \Delta x + B \cdot \Delta y \right)\\
\frac{\partial G_i}{\partial \Delta y}&=-G_i \left( B \cdot \Delta x + C \cdot \Delta y \right)
\end{aligned}
$$
...
$$
\begin{aligned}
\frac{\partial L}{\partial x_i}&=\frac{\partial L}{\partial G_i} \cdot \frac{\partial G_i}{\partial \Delta x} \cdot \frac{\partial \Delta x}{\partial x_i}&=\frac{\partial L}{\partial G_i} \cdot \left( -G_i \left( A \cdot \Delta x + B \cdot \Delta y \right) \right)\\
\frac{\partial L}{\partial y_i}&=\frac{\partial L}{\partial G_i} \cdot \frac{\partial G_i}{\partial \Delta y} \cdot \frac{\partial \Delta y}{\partial y_i}&=\frac{\partial L}{\partial G_i} \cdot \left( -G_i \left( B \cdot \Delta x + C \cdot \Delta y \right) \right)
\end{aligned}
$$
