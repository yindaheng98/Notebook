...
具体来说，因为 PGSR 人为规定了一个短轴方向，所以这个短轴方向就是法线，根据法线方向和 Gaussian 位置，每个 Gaussian 都可以写出一个平面方程。
对于从相机发出的每个射线，其相交的 Gaussian 有这么一个平面方程，PGSR 的策略是先对这些平面方程的参数求alpha-blending，再求这个alpha-blending后的“平均平面”和射线的交点。

![](i/PGSRBlending.png)

由于求射线相交的计算过程中有个除法，平面方程的参数求alpha-blending之后进入这个除法会导致alpha-blending算出的透明度项被抵消。
