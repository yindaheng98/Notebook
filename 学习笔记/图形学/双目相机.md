...
以$\bm E=(\bm K^{-1})^T(\bm t\times\bm R)\bm K^{-1}$为例，此时的$\bm x,\bm x'$是目标点在相机坐标系下的坐标，可见对极约束方程的物理含义。
如图所示：

![](i/epipolarphy.png)

简言之，此处先回忆本科学的解析几何知识：“空间中的点$\bm x=[x,y,z]^T$位于平面$\bm l=[a,b,c]^T$上”等价于$ax+by+cz=0$。所以在像平面不确定的情况下，$\bm l=[a,b,c]^T$实际上描述了极平面位置，而显然像平面方程为“$z=\text{常数}$”的形式，其与极平面相交即为极线$ax+by=\text{常数}$。
