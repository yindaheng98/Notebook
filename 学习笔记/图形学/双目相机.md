...
根据$\bm a\cdot\bm b=\bm a^T\bm b=\bm b^T\bm a$写成矩阵形式并把括号里面的$\bm x_1$拿出来：

$$\bm x_2^T(\bm t\times\bm R)\bm x_1=0$$

此即对极约束方程。或重新带入$\bm x,\bm x'$：

$$\bm x'^T(\bm K^{-1})^T(\bm t\times\bm R)\bm K^{-1}\bm x=0$$
...
其中的$\bm E$称为“本质矩阵”（Essential Matrix）。在上述对极约束方程中，本质矩阵为$\bm E=\bm t\times\bm R$或$\bm E=(\bm K^{-1})^T(\bm t\times\bm R)\bm K^{-1}$。其性质可以表示为：
