...
$$\bm x'^T(\bm K^{-1})^T(\bm t_\times\bm R)\bm K^{-1}\bm x=0$$
...
其中的$\bm E$称为“本质矩阵”（Essential Matrix）。在上述对极约束方程中，本质矩阵为$\bm E=\bm t_\times\bm R$或$\bm E=(\bm K^{-1})^T(\bm t_\times\bm R)\bm K^{-1}$。其性质可以表示为：
...
虽然$\bm E=\bm t\times\bm R$和$\bm E=(\bm K^{-1})^T(\bm t_\times\bm R)\bm K^{-1}$都是本质矩阵，但$\bm E=(\bm K^{-1})^T(\bm t_\times\bm R)\bm K^{-1}$通常叫做基础矩阵(fundamental matrix)，记为$\bm F$。

### 求基础矩阵：八点算法 8-point algorithm

求解基础矩阵实际上是在求$\bm R,\bm t$，即左右两相机的相对位姿。
...
$$
x'xf_1+x'yf_2+x'zf_3+y'xf_4+y'xf_5+y'zf_6+z'xf_7+z'yf_8+z'zf_9=0
$$

每一对点就有一个方程，所以共需要9对点，那需要18个点吗？并不，9对点并不需要完全不一样，只需要**保证9个方程线性无关**即可求解，实际上只需要8个点。（为什么？）
