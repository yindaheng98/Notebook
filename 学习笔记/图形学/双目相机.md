...
$$
\begin{aligned}
{\bm{x}_1} = {\bm{K}^{ - 1}}{\bm x}&=\bm X\\
{\bm{x}_2} = {\bm{K}^{ - 1}}{\bm x'}&=\bm R\bm X+\bm t\\
\end{aligned}
$$
...
构造一个$\bm t$的斜对称行列式$\bm T$，可使得$\bm T\cdot\bm t=\bm 0$（其实就是向量叉积$\bm t\times\bm t=0$）：
...
两边都乘上$\bm T$，$\bm T\cdot\bm t=\bm 0$为0被消掉：

$$\bm T\bm{x}_2 = \bm T\bm{R} {\bm{x}_1}$$

两边都乘上$\bm x_2^T$：

$$\bm x_2^T\bm T\bm{x}_2 = \bm x_2^T\bm T\bm{R} {\bm{x}_1}$$

而：

$$
\begin{aligned}
\bm x_2^T\bm T\bm{x}_2
&=
[x_2,y_2,z_2]
\cdot
\left[
...
\left[
    \begin{matrix}
        t_2z_2-t_3y_2\\t_3x_2-t_1z_2\\t_1y_2-t_2x_2
    \end{matrix}
\right]
\\
&=
x_2(t_2z_2-t_3y_2)+y_2(t_3x_2-t_1z_2)+z_2(t_1y_2-t_2x_2)
\\
&=0
\end{aligned}
$$
...
$$\bm x_2^T\bm T\bm{R} {\bm{x}_1}=0$$

此即对极约束方程。或重新带入$\bm x,\bm x'$：

$$\bm x'^T(\bm K^{-1})^T\bm T\bm{R}\bm K^{-1}\bm x=0$$
...
其中的$\bm E$称为“本质矩阵”（Essential Matrix）。在上述对极约束方程中，本质矩阵为$\bm E=\bm T\bm{R}$或$\bm E=(\bm K^{-1})^T\bm T\bm{R}\bm K^{-1}$。其性质可以表示为：
...
以$\bm E=(\bm K^{-1})^T\bm T\bm{R}\bm K^{-1}$为例，此时的$\bm x,\bm x'$是目标点在相机坐标系下的坐标，可见对极约束方程的物理含义。
具体来说，“两向量点积为0$\Rightarrow$两向量正交”
如图所示：
