...
$$
(\exist D=(Q,\Sigma,\delta,q_0,F))(L=\bm L(D))
$$
...
4. （递归）如果$\bm r$、$\bm s$都是正则表达式，分别表示语言$R$和$S$，那么：
    * （并运算）$\bm r+\bm s$是正则表达式，表示语言$R\cup S$
    * （连接运算）$\bm r\bm s$是正则表达式，表示语言$RS$
    * （闭包运算）$\bm r^*$是正则表达式，表示语言$R^*$
    * （改变运算优先级）$(\bm r)$是正则表达式，表示语言$R$
...
注：实际计算中当然不不可能遍历所有字符串，具体使用的构造方法有递归式法、状态消除法等。
